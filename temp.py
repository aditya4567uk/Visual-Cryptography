...
def convertToRGB(img):
	try:
		...
		print("[red]uable convert image to RGB ,something is wrong [/red]- %s"%e)
...
def encodeImage(image,message,filename):
	with console.status("[cyan]Encoding image plz wait..") as status:
		try:
			...
			print("\n")
			print("[yellow]Name of Original File: [u]%s[/u][/yellow]"%filename)
			print("[green]Image encoding SUCCESS and saved as [u][bold]%s[/green][/u][/bold]"%encoded_filename)
...
def decodeImage(image):
	with console.status("[yellow]Decoding image..") as status:
		try:
			pix = image.getdata()
			current_pixel = 0
			decoded=""
...
def print_credits():
    ...
    table.add_column("Project_Members_Name",style="red")
    table.add_column("REG_NO",style="red")
    table.add_row("Aditya Kumar", "20MIP10008")
    table.add_row("Akanksha Verma","20MIP10002")
    table.add_row("Ishika Shrivastava","20MIP10035")
    table.add_row("Charchit Jain","20MIP10018")
    table.add_row("Ayush Manoj","20MIM10118")
    table.add_row("Nitish Kumar","20MIM10117")
    console.print(table)
...
def main():
	...
	print("[yellow]Press 1 to ENCODE & PRESS 2 TO DECODE: [/yellow]")
	op = int(input("1. Encode\n2. Decode\n>>"))

	if op==1:
		print("[green]Image path (with extension): [/green]")
		img = input(">>")
		if(not(path.exists(img))):
			raise Exception("Image not found!")

		
		print("[yellow]Message to be hidden: [/yellow]")
		message = input(">>")
		...
		password=""
		while 1:
			print("[yellow]Password to encrypt: [/yellow]")
			password = getpass.getpass(">>")
			if password=="":
				break
			print("[yellow]Enter Password Again: [/yellow]")
			confirm_password = getpass.getpass(">>")
			if(password!=confirm_password):
				print("[red]Passwords is wrong try again [/red]")
			else:
				break
		...
		print("[green]provide image file path with extension: [/green]")
		img = input(">>")
		if(not(path.exists(img))):
			raise Exception("Image not found!")

		print("[green]Enter password : [/green]")
		password = getpass.getpass(">>")
...
if __name__ == "__main__":
	...
	cprint(figlet_format('IMG_ENCRYPTER', font='starwars'),'green', attrs=['bold'])
	print_credits()
	print("[bold]IMG_ENCRYPTER[/bold] POWERFUL PROGRAM TO ENCRYPT TEXTUAL DATA IN AN IMAGE FILE WHICH USES AES-SHA256 PROTECTION.")
	print()
	main()